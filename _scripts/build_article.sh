...
git checkout -b posts/$FILENAME
git add "../_posts/$FILENAME"
git commit -m "[CF-Pages-Skip] ${TITLE}" 
git push origin posts/$FILENAME --force

gh pr create \
  --title "[CF-Pages-Skip] ${TITLE}" \
  --body "[CF-Pages-Skip] this PR is generated by github-actions[bot]" \
  --base main \
  --head posts/$FILENAME